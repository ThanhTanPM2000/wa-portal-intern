...
import { AccountSearchItem, AccountSearchResult, RequestWithUserInfo, UserStatusEnum } from '../types';
import { facebook, account, partnerKey, user, audit } from '../services';
...
export const connect = async (req: RequestWithUserInfo, res: Response): Promise<void | Response> => {
  try {
    ...
    let partnerKeyId: number | undefined;
    let partnerId: number | undefined;
    if (body.partnerKey) {
      const token = await partnerKey.getValidPartnerKeyByValue(body.partnerKey);
      if (!token) {
        return res.status(StatusCodes.NOT_FOUND).send();
      }
      partnerKeyId = token.id;
      partnerId = token.partnerId;
    }

    await audit.insertAudit(AuditAction.WABA_CONNECTION_REQUEST, {
      email: req.user.email,
      partnerToken: '' + body.partnerKey,
      partnerId: '' + partnerId,
    });
...
export const connectWithCustomerId = async (req: Request, res: Response): Promise<void | Response> => {
  try {
    ...
    const { partnerKey: partnerKeyToConnect, customerId, oauthToken } = body;
    const token = await partnerKey.getValidPartnerKeyByValue(partnerKeyToConnect);
    if (!token) {
      return res.status(StatusCodes.NOT_FOUND).send();
    }
    const partnerKeyId = token.id;
...
export const find = async (req: RequestWithUserInfo, res: Response): Promise<void | Response> => {
  try {
    ...
    const {
      ...
      page: selectedPage,
      size: selectedSize,
    }: AccountSearchResult = await account.findAccounts(isAdmin ? undefined : id, !!is_getting_all, page, size, search);
    const sanitizedAccounts = accounts.map((account: AccountSearchItem) => {
      return {
        id: account.id,

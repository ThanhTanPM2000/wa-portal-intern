...
export const getAvailableServiceProviders = async (req: RequestWithUserInfo, res: Response): Promise<void | Response> => {
  try {
    const partnerId = req.user.partner?.id;
    if(!partnerId) return res.status(StatusCodes.UNAUTHORIZED).send()
    const availableServiceProviders = await services.partner.getAvailableServiceProviders(partnerId);
    return res.send(availableServiceProviders);
  } catch (error) {
    logger.error(error.message)
    return res.status(StatusCodes.BAD_REQUEST).send()
  }

...
export const initiateOnboardingProcess = async (req: RequestWithUserInfo, res: Response): Promise<void | Response> => {
  try {
    ...
    await services.audit.insertAudit(AuditAction.WABA_CONNECT_FLOW_STARTED, {
      userId: '' + req.user.id
    });
    return res.send();
  } catch (error) {
...
export const initiateOnboardingProcessByCustomer = async (req: Request, res: Response): Promise<void | Response> => {
  try {
    const { body: {customerId} } = validators.initiateOnboardingProcessByCustomer(req);
    const user = await services.user.findByCustomerId(customerId);
    if(!user) throw new Error(`Cannot find User with customerId ${customerId}`)
    await services.audit.insertAudit(AuditAction.WABA_CONNECT_FLOW_STARTED, {
      userId: '' + user.id,
      isUsingCustomerId: "true",
    });
    await services.user.changeOnboardingStatus(user.id, UserStatusEnum.Initiated);
    return res.send();
  } catch (error) {
    logger.error(error.message);
    return res.status(StatusCodes.BAD_REQUEST).send();

...
import { createPartnerKey } from './partnerKey';
...
export const getEmbeddedUrl = async (partnerId: number): Promise<string> => {
  ...
  let partnerKey: any = await prisma.partnerKey.findFirst({
    where: {
      partnerId,
      revokedAt: null,
  ...
  if (!partnerKey) {
    partnerKey = await createPartnerKey(partnerId);
  }

  const embeddedUrl = `${config.CLIENT_URL}/wa-client?partner=${partnerKey.value}`;
  return embeddedUrl;
};

export const getAvailableServiceProviders = async (partnerId: number) => {

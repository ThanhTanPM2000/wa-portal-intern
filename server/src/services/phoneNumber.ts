...
export const findPhoneNumbers = async (userId?: number, isGettingAll = false, page = 1, size = 10, search = '') => {
  try {
    ...
    const phoneNumbers = await prisma.phoneNumber.findMany({
      ...
      include: { account: true, provisionServer: true },
    });
...
export const getPhoneCert = async (phoneNumberId: string) => {
  try {
    let phoneCert;
    const cacheKey = `${phoneNumberId}:cert`;
    phoneCert = await analyticCacheManager.getCache(cacheKey);
    if (!phoneCert || !phoneCert.certificate) {
      phoneCert = await api.getPhoneNumberCert(phoneNumberId);

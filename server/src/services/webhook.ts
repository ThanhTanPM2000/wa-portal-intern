...
const handlePhoneNumberQualityUpdate = async (value: never): Promise<void> => {
  ...
  const phoneNumber = await prisma.phoneNumber.findFirst({
    where: {
      OR: [{ shortenValue: display_phone_number }, { value: display_phone_number }],
    },
  });
  if (!phoneNumber) {
...
export const handleProvisionServerStatusUpdate = async (nameProvider: string, dataWebhook: any) => {
  try {
    const { phone_number } = dataWebhook;
    const phoneNumber = await prisma.phoneNumber.findFirst({
      where: {
        shortenValue: phone_number,
      },
    });

    if (!phoneNumber) return;

    findProvision(nameProvider)?.handleStatusUpdateWebhook(phoneNumber.id, dataWebhook);
  } catch (error) {
    logger.error(error.message);
    return;
  }

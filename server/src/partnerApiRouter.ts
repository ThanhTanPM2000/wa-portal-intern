...
import { account, partner, partnerKey, phoneNumber, user } from './controllers';
...
const partnerApiRouter = (): Router => {
  ...
  router.get('/partner-key', apiKeyMiddleware, partnerKey.find);
  router.post('/partner-key', apiKeyMiddleware, partnerKey.create);
  router.put('/partner-key/:partnerKeyId/revoke', apiKeyMiddleware, partnerKey.revoke);
